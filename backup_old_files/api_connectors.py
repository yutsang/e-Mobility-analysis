...
class KMBLWBConnector:
    ...
    def get_route_stops(self, route_id: str, direction: int = 1, service_type: int = 1) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"Error fetching KMB route stops for {route_id} from database: {e}")
            return pd.DataFrame()
    
    def get_stop_eta(self, stop_id: str, route_id: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"KMB Stop ETA API failed with status {response.status_code}: {response.text}")
