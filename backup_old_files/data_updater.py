...
class KMBDataUpdater:
    ...
    def fetch_route_stops(self, route_id: str, direction: str = "outbound", service_type: int = 1) -> List[Dict]:
        """
    ...
    def fetch_all_route_stops(self, routes: List[Dict]) -> List[Dict]:
        ...
        for i, route in enumerate(routes):
            route_id = route.get('route_id')  # Database field is 'route_id', not 'route'
            if not route_id:
                continue
                
            # Fetch for both directions
            for direction_name, direction_bound in [("outbound", "O"), ("inbound", "I")]:
    ...
    def update_routes(self) -> bool:
        ...
        try:
            ...
            if not routes:
                self.db_manager.log_update('routes', 0, 'error', 'No routes data fetched')
                return False
    ...
    def update_route_stops(self, max_routes: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if not route_stops:
                self.db_manager.log_update('route_stops', 0, 'error', 'No route-stops data fetched')
                return False
...
def main():
    ...
    parser.add_argument('--route-stops', action='store_true', help='Update route-stops only')
    parser.add_argument('--all', action='store_true', help='Update all data')
    parser.add_argument('--max-routes', type=int, help='Maximum number of routes to process')
    ...
    if args.routes:
        ...
        print("Please specify what to update: --routes, --stops, --route-stops, --all, or --status")
