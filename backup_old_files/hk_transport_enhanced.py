    ...
    def get_kmb_data(self):
        ...
        try:
            ...
            if 'KMB/LWB' in stops_data and not stops_data['KMB/LWB'].empty:
                ...
                st.warning("⚠️ No KMB stops data available in local database. Please run the data updater to populate the database.")
                return pd.DataFrame()
    ...
    def get_kmb_routes(self):
        ...
        try:
            ...
            if 'KMB/LWB' in routes_data and not routes_data['KMB/LWB'].empty:
                ...
                # Show database status if no data
                st.warning("⚠️ No KMB routes data available in local database. Please run the data updater to populate the database.")
                return pd.DataFrame()
        except Exception as e:
...
def create_dashboard():
    ...
    try:
        ...
        if stats['routes_count'] > 0 and stats['stops_count'] > 0:
            ...
            st.sidebar.warning("Run `python data_updater.py --all` to populate the database")
    except Exception as e:
        ...
        if not transport_data_dict.get('KMB', pd.DataFrame()).empty:
            ...
            with col1:
                ...
                if not routes_data.empty:
                    ...
                    service_counts = routes_data.groupby('service_type').size().reset_index(name='count')
                    fig = px.pie(
                        service_counts,
                        values='count',
                        names='service_type',
                        title="Routes by Service Type",
                        color_discrete_map={'1': '#0066cc', '2': '#0080ff', '3': '#3399ff'}
                    )
    ...
    with tab3:
        ...
        if not routes_data.empty:
            ...
            if selected_route:
                route_info = routes_data[routes_data['route_id'] == selected_route].iloc[0]
    ...
    with tab4:
        ...
        with col1:
            ...
            hours = list(range(24))
            passenger_count = [50, 30, 20, 10, 15, 40, 180, 350, 520, 480, 420, 380, 
                              400, 450, 480, 520, 600, 680, 650, 550, 450, 350, 200, 100]
