...
sys.path.append(os.path.join(os.path.dirname(__file__), 'src', 'hk_kmb_transport', 'pipelines', 'web_app'))
...
def debug_missing_routes():
    ...
    for route in missing_routes:
        ...
        if not route_data.empty:
            print(f"✅ Route {route} found: {route_data.iloc[0]['origin']} → {route_data.iloc[0]['destination']}")
        else:
