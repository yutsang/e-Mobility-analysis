...
def load_kmb_data():
    ...
    try:
        ...
        return routes.get('KMB/LWB', pd.DataFrame()), stops.get('KMB/LWB', pd.DataFrame())
        ...
        for idx, stop in route_stops.iterrows():
            if pd.notna(stop['lat']) and pd.notna(stop['lng']):
                ...
                folium.Marker(
                    ...
                    icon=folium.Icon(color=marker_color, icon_color=icon_color, icon='bus', prefix='fa')
                ).add_to(m)
...
def main():
    ...
    st.markdown('<h1 class="main-header">🚌 Hong Kong KMB Route Map</h1>', unsafe_allow_html=True)
    ...
    if not routes_df.empty:
        ...
        for idx, route in routes_df.iterrows():
            ...
            route_options.append((route_display, route['route_id'], 1, route.get('service_type', 1)))
        
        selected_route_display = st.sidebar.selectbox(
            "Select Route",
            ["None"] + [opt[0] for opt in route_options]
        )
        ...
        if selected_route_display != "None":
            ...
            if selected_route_id:
                with st.spinner("Loading route stops..."):
                    route_stops = get_route_stops(selected_route_id, selected_direction, selected_service_type)
                
                if not route_stops.empty:
                    # Stop selection
                    stop_options = [f"Stop {row['sequence']}: {row['stop_name']}" for idx, row in route_stops.iterrows()]
                    ...
                    if selected_stop_display != "None":
                        ...
                        selected_stop_row = route_stops[route_stops['sequence'] == stop_seq]
    ...
    if not route_stops.empty:
        ...
        default_map = folium.Map(location=HK_CENTER, zoom_start=11, tiles='OpenStreetMap')
