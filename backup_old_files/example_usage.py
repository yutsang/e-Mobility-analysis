...
def demonstrate_system():
    ...
    if not update_history.empty:
        ...
        for i, update in update_history.iterrows():
            print(f"  {update['updated_at']}: {update['update_type']} - {update['status']} ({update['records_updated']} records)")
    else:
...
def main():
    ...
    parser.add_argument('--update', action='store_true', help='Update database before demo')
