...
def create_enhanced_map(transport_data, selected_route_stops=None, selected_stop=None, show_traffic=False):
...
def create_dashboard():
    ...
    st.markdown('<h1 class="main-header">🚇 Hong Kong Real-time Transportation Dashboard</h1>', unsafe_allow_html=True)
    ...
    if transport_mode != "All" and transport_mode in all_routes:
        ...
        if not routes_df.empty:
            route_options = [f"{row['route_id']} - {row['route_name']}" for idx, row in routes_df.iterrows()]
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🗺️ Interactive Map", "🚌 Route & ETA", "📊 Statistics", "🚦 Service Status"])
    ...
    with tab1:
        ...
        if transport_mode == "All":
            ...
            display_data = {transport_mode: all_stops.get(transport_mode, pd.DataFrame())}
    ...
    with tab2:
        ...
        if selected_route_id and selected_stop_id:
            ...
            st.info("Please select a transport mode, route, and stop to view real-time ETA information.")
    ...
    with tab3:
        ...
        if any(not df.empty for df in all_stops.values()):
            ...
            with col1:
                ...
                transport_counts = {company: len(df) for company, df in all_stops.items() if not df.empty}
                ...
                if transport_counts:
                    fig = px.pie(
                        ...
                        color_discrete_map={'MTR': '#ff0000', 'KMB/LWB': '#0000ff', 'Citybus': '#00ff00', 'GMB': '#ffa500'}
                    )
