...
def create_dashboard():
    ...
    st.markdown('<h1 class="main-header">🚌 Hong Kong KMB Bus Dashboard</h1>', unsafe_allow_html=True)
    ...
    if 'KMB/LWB' in all_routes:
        ...
        if not routes_df.empty:
            ...
            for idx, row in routes_df.iterrows():
                ...
                if route_key not in seen_routes:
                    ...
                    route_display = f"{row['route_id']} - {row['origin']} → {row['destination']}"
                ...
                if selected_route:
                    ...
                    if len(route_variants) > 1:
                        ...
                        for _, variant in route_variants.iterrows():
                            bound_text = "Outbound" if variant.get('bound') == 'O' else "Inbound"
                            direction_options.append(f"{bound_text} (Service {variant.get('service_type', '1')})")
                        ...
                        selected_variant_idx = direction_options.index(selected_direction_display)
        ...
        if selected_route_id:
            with st.spinner("Loading route stops..."):
                route_stops = get_cached_route_stops(selected_route_id, direction, service_type)
                if not route_stops.empty:
                    stop_options = [f"{row['stop_name']} (Seq: {row['sequence']})" for idx, row in route_stops.iterrows()]
                    ...
                    if selected_stop_display != "None":
                        ...
                        selected_stop_row = route_stops[route_stops['stop_name'] == selected_stop_name].iloc[0]
    ...
    with col2:
        ...
        if not all_routes.get('KMB/LWB', pd.DataFrame()).empty:
            routes_df = all_routes['KMB/LWB']
            service_counts = routes_df.groupby('service_type').size().reset_index(name='count')
            fig = px.bar(
                service_counts, 
                x='service_type', 
                y='count', 
    ...
    if selected_route_id and selected_stop_id:
        ...
        with col1:
            ...
            if selected_stop_row is not None:
                ...
                st.write(f"**Coordinates:** {selected_stop_row['lat']:.6f}, {selected_stop_row['lng']:.6f}")
        ...
        if not route_stops.empty:
            ...
            for idx, stop_row in route_stops.iterrows():
                ...
                route_display_data.append({
                    ...
                    'Type': '🚌 Depot' if 'Depot' in stop_row['stop_name'] or stop_row['sequence'] in [1, len(route_stops)] else '📍 Stop'
                })
            ...
            if len(route_stops) >= 2:
                ...
                with col1:
                    ...
                    st.write(f"Location: {first_stop['lat']:.6f}, {first_stop['lng']:.6f}")
                
                with col2:
                    st.info(f"**Ending Point:** {last_stop['stop_name']}")
                    st.write(f"Location: {last_stop['lat']:.6f}, {last_stop['lng']:.6f}")
        ...
        if not route_stops.empty:
            ...
            route_stops_display = route_stops[['sequence', 'stop_name', 'stop_id']].copy()
        ...
        st.info("💡 **Tip**: Select a KMB route to view detailed information and see stops on the map.")
