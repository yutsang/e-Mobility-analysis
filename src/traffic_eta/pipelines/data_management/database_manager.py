...
class KMBDatabaseManager:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_routes_route_id ON routes(route_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_stops_stop_id ON stops(stop_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_route_stops_route_id ON route_stops(route_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_route_stops_stop_id ON route_stops(stop_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_route_stops_direction ON route_stops(direction)')
    ...
    def get_route_stops(self, route_id: str, direction: int = 1, service_type: int = 1) -> pd.DataFrame:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            '''
            return pd.read_sql_query(query, conn, params=(route_id, direction, service_type))
    ...
    def log_update(self, update_type: str, records_updated: int, status: str, error_message: Optional[str] = None):
        """
