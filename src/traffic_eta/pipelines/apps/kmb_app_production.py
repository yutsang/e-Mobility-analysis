...
def main():
    ...
    st.markdown('<h1 class="main-header">🚌 Hong Kong KMB Transport</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; opacity: 0.7; margin-bottom: 2rem;">Production-ready route explorer with complete data coverage</p>', unsafe_allow_html=True)
    ...
    if search_term:
        mask = (
            ...
            sorted_routes['destination'].str.contains(search_term, case=False, na=False)
        )
    ...
    if not filtered_routes.empty:
        route_options = [format_route_option(row) for idx, row in filtered_routes.iterrows()]
        ...
        if selected_route_display != "None":
            ...
            selected_route_info = filtered_routes[filtered_routes['route_id'] == selected_route_id].iloc[0]
            ...
            if not route_stops.empty:
                ...
                if len(available_directions) > 1:
                    direction_options = [f"{d['name']} ({d['stops']} stops)" for d in available_directions]
                    selected_direction_display = st.sidebar.selectbox(
                        "Select Direction",
                        direction_options,
                        help="Choose route direction"
                    )
                    selected_direction = available_directions[direction_options.index(selected_direction_display)]['direction']
                else:
                ...
                direction_stops = route_stops[route_stops['direction'] == selected_direction].sort_values('sequence')
                if not direction_stops.empty:
                    stop_options = [f"Stop {row['sequence']}: {row['stop_name']}" for idx, row in direction_stops.iterrows()]
                    selected_stop_display = st.sidebar.selectbox(
                        "Highlight Stop (Optional)",
                        ["None"] + stop_options
                    )
    ...
    if selected_route_id and not route_stops.empty:
        ...
        direction_stops_count = len(route_stops[route_stops['direction'] == selected_direction])
        ...
        for d in available_directions:
            ...
            direction_badges.append(f'<span class="direction-badge {badge_class}">{active}{d["name"]} ({d["stops"]} stops)</span>')
        
        st.markdown(f"**Available Directions:** {''.join(direction_badges)}", unsafe_allow_html=True)
        
        # Map display
        st.header("🗺️ Interactive Route Map")
        
        # Create and display map
        map_obj = create_enhanced_route_map(route_stops, selected_stop_id, selected_direction)
        ...
        st.info("👆 Select a route from the sidebar to view its path, stops, and real-time information.")
        ...
        with col3:
            ...
            routes_with_stops = routes_df[routes_df['route_id'].isin([
                route_id for route_id in routes_df['route_id'] 
        ...
        import folium
        default_map = folium.Map(location=[22.3193, 114.1694], zoom_start=11, tiles='OpenStreetMap')
