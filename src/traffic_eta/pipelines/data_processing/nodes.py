...
def validate_database_integrity() -> Dict:
    ...
    try:
        ...
        if stats['routes_count'] < 100:
            validation_results['issues'].append(f"Low route count: {stats['routes_count']}")
        
        if stats['stops_count'] < 1000:
            validation_results['issues'].append(f"Low stop count: {stats['stops_count']}")
        
        if stats['route_stops_count'] < 5000:
            validation_results['issues'].append(f"Low route-stop count: {stats['route_stops_count']}")
        
        # Check for orphaned route-stops
        orphaned_check = db_manager.check_data_integrity()
        if orphaned_check['orphaned_route_stops'] > 0:
            validation_results['issues'].append(f"Found {orphaned_check['orphaned_route_stops']} orphaned route-stops")
        
        if validation_results['issues']:
            validation_results['is_valid'] = False
        
        logger.info(f"Database validation completed. Valid: {validation_results['is_valid']}")
...
def create_sample_data_for_testing() -> Dict:
    ...
    try:
        ...
        sample_routes = [
            {'route': '65X', 'dest_en': 'Tsim Sha Tsui (Circular)', 'orig_en': 'Tin Shui Wai (Tin Yiu Bus Terminus)', 'service_type': 1},
            {'route': '219X', 'dest_en': 'Tsim Sha Tsui (Circular)', 'orig_en': 'Ko Ling Road', 'service_type': 1},
            {'route': '24', 'dest_en': 'Mong Kok (Circular)', 'orig_en': 'Kai Yip', 'service_type': 1},
            {'route': 'E23', 'dest_en': 'Airport (Ground Transportation Centre)', 'orig_en': 'Tsim Sha Tsui East', 'service_type': 1},
            {'route': '41A', 'dest_en': 'Tsim Sha Tsui East', 'orig_en': 'Ma On Shan (Heng On)', 'service_type': 1}
        ]
        
        # Sample stops
        sample_stops = [
            {'stop': 'STOP_TST_001', 'name_en': 'Tsim Sha Tsui (Nathan Road)', 'lat': 22.2976, 'long': 114.1697},
            {'stop': 'STOP_TSW_001', 'name_en': 'Tin Shui Wai Station', 'lat': 22.4578, 'long': 113.9938},
            {'stop': 'STOP_MK_001', 'name_en': 'Mong Kok (Argyle Street)', 'lat': 22.3193, 'long': 114.1694},
            {'stop': 'STOP_AP_001', 'name_en': 'Airport Terminal 1', 'lat': 22.3080, 'long': 113.9185}
        ]
        
        # Sample route-stops
        sample_route_stops = [
            {'route': '65X', 'stop': 'STOP_TSW_001', 'bound': 'O', 'seq': 1, 'service_type': 1},
            {'route': '65X', 'stop': 'STOP_TST_001', 'bound': 'O', 'seq': 2, 'service_type': 1},
            {'route': '219X', 'stop': 'STOP_TST_001', 'bound': 'O', 'seq': 1, 'service_type': 1},
            {'route': '24', 'stop': 'STOP_MK_001', 'bound': 'O', 'seq': 1, 'service_type': 1}
        ]
        ...
        logger.info(f"Created sample data: {routes_count} routes, {stops_count} stops, {route_stops_count} route-stops")
