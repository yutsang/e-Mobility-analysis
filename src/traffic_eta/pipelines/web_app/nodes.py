    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            directions_query = """
                ...
                ORDER BY direction
            """
            directions_df = pd.read_sql_query(directions_query, conn, params=(route_id,))
...
def search_routes_with_directions(routes_df: pd.DataFrame, search_term: str) -> List[Dict]:
...
def get_osm_route_with_waypoints(stops_coords: List[Tuple[float, float]], max_waypoints: int = MAX_WAYPOINTS) -> List[List[float]]:
...
def get_single_osm_route(stops_coords: List[Tuple[float, float]]) -> Optional[List[List[float]]]:
...
def get_route_geometry_with_progress(route_stops: pd.DataFrame, direction: int) -> List[List[float]]:
...
def create_enhanced_route_map(route_stops: pd.DataFrame, selected_stop_id: Optional[str] = None, direction: int = 1) -> folium.Map:
...
def should_update_data() -> bool:
    ...
    if params["schedule"]["daily_update"]["enabled"]:
        ...
        update_time = datetime.datetime.strptime(params["schedule"]["daily_update"]["time"], "%H:%M").time()
